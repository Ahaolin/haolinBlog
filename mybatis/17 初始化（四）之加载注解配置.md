...
`#parseCacheRef()` 方法，解析 `@CacheNamespaceRef` 注解。[->](#jump2.2_6)代码如下：
...
- `<2>`处，如果SQL_ANNOTATION_TYPES对应的类型非空的情况，例如`@Insert`注解：
...
`#getReturnType(Method method)` 方法，获得返回类型。[->](#jump2.6.6_1) 代码如下：
